...
- Última interação relevante: 2026-05-07 — Resgate step 6 vencido, mas não enviado; ciclo pausado por duplicidade/ownership
- Último envio real de WhatsApp: 2026-05-03 — Resgate step 5 enviado
...
### Momento
Cliente está em `Em Atendimento`, com silêncio após outbound normal e depois após cinco steps de Resgate. O ciclo foi pausado antes do step 6 por revisão de ownership/duplicidade.
...
### 2026-05-07 — Fila vencida revalidada; Resgate não enviado
A revalidação encontrou duplicidade/ownership recente no mesmo telefone/JID em cadastro SLA Cascata ativo: cliente 11129, broker 24, `source_details.cliente_original_id=11013`. Como há outro cadastro ativo recente com o mesmo contato, o envio foi bloqueado antes do step 6 e o ciclo foi pausado com `stopped_reason=manual_review_duplicate_active_broker`.
...
- Duplicidade/ownership recente em cadastro SLA Cascata ativo com o mesmo telefone/JID exige revisão manual antes de qualquer novo envio.

## Próximo passo
Manter o Resgate pausado até revisão manual do ownership/duplicidade. Não enviar novos steps enquanto `stopped_reason=manual_review_duplicate_active_broker`. O status comercial permanece `Em Atendimento` e a branch de Resgate foi encerrada operacionalmente nesta execução sem WhatsApp.
...
- Em 2026-05-07 o worker não enviou step 6 porque encontrou duplicidade/ownership recente no mesmo telefone/JID em outro cadastro SLA Cascata ativo; a branch foi pausada com `enabled=false`, `next_run_at=null` e `stopped_reason=manual_review_duplicate_active_broker`.
